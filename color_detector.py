...
# Load the image
image_path = Path('corrosion_pictures/13.png')
image = cv2.imread(image_path)
hight, wide, m = image.shape

output_dir = Path('processed_pictures')
output_dir.mkdir(exist_ok=True)

properties = float (sys.argv[1]) # gal/m2 

reference = float (sys.argv[2]) # meters
...
cv2.imshow("Corrosion detection", image)
cv2.waitKey(0)
cv2.destroyAllWindows()
# Ensure the mask is single-channel
if len(mask_colored.shape) == 3:
    mask_colored = cv2.cvtColor(mask_colored, cv2.COLOR_BGR2GRAY)
# Calculate the area of the mask
mask_area = cv2.countNonZero(mask_colored)
print(f"If the hight of the image represent {reference} meters,\nand the properties of the paint specify {properties} gal/m2,\nYou will need: {round (mask_area*(reference/hight)**2*properties, 2)} gals of paint")
